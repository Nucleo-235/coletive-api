...
class TrelloProject < Project
  ...
  def participate(new_member_user)
    if new_member_user.trello_member && new_member_user.trello_member_id
      trello_members = Trello::Member.from_response user.trello_client.get("/boards/#{info.board_id}/members")
      trello_members_ids = trello_members.map { |member| member.id  }
      if !trello_members_ids.include? new_member_user.trello_member_id
        response = user.trello_client.get("/boards/#{info.board_id}/members/#{new_member_user.trello_member_id}")
        # puts response
      end
      true
    else
